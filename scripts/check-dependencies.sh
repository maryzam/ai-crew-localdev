...
allow_provider_contracts() {
	...
	local label=$2
	local unexpected
	unexpected=$(go list -buildvcs=false -f '{{range .Imports}}{{println .}}{{end}}' $packages | grep -E "^${module}/internal/providers/" | grep -Ev "^${module}/internal/providers/([a-z]+/contract|capabilities)$" || true)
	if [ -n "$unexpected" ]; then
		printf 'dependency boundary failed: %s\n%s\n' "$label" "$unexpected" >&2
		failed=1
	fi
...
allow_internal_imports "./internal/broker/policycheck" "^${module}/internal/(broker/core|configmodel/(identity|policy)|providers/catalog)$" "broker policy validation adapter may compose only broker core, config models, and provider catalog"
