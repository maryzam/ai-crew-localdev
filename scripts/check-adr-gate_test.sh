...
policy_head_sha="$(git rev-parse HEAD)"

if "$gate" --range "$base_sha" "$policy_head_sha" --body-file "$empty_body" >"$gate_log" 2>&1; then
...
if "$gate" --range "$base_sha" "$policy_head_sha" --body-file "$opt_out_body" >"$gate_log" 2>&1; then
...
if "$gate" --range "$base_sha" "$broker_head_sha" --body-file "$empty_body" >"$gate_log" 2>&1; then
