...
- Learning loop: retained run telemetry now feeds a bounded advisory analyzer that produces workflow recommendations without mutation. North-star work makes that loop act: findings persist in a durable ledger with acceptance status so later analysis reports measured outcome deltas, run-level token budgets act on live usage during the run, and approved recommendations connect to the future manifest and cockpit.
