...
- The containment P1 is closed by explicit, tested claim boundaries rather than an adversarial-sandbox claim. Managed runs reject ordinary host-native execution, brokered tools fail closed on the supported path, durable secrets stay host-side, and the user/security docs state that raw network egress, absolute-path tool bypasses, spoofed environment markers, and same-UID compromise are outside the containment claim.
...
The repository can claim a governed local substrate for AI coding agents: broker-retained durable provider secrets, scoped GitHub credentials, fail-closed brokered tooling on the supported path, ordinary host-native managed-run rejection, project-aware managed runs, project operating-model manifest enforcement for supported declarations, first-use smoke coverage, bounded quality evidence, local run history, native usage capture, live token budgets, and advisory adaptive findings.
