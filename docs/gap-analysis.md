...
| P1 | Installation from a release exists but the clean-host journey is unproven. | Tag-triggered releases build a static multi-call binary for amd64 and arm64 with `SHA256SUMS`, and `install.sh` verifies checksums fail-closed and creates the invocation symlinks; an executable test in `make verify` proves valid installs and refuses tampered or unchecksummed artifacts, and the release workflow smoke-tests the real artifact through the installer. No clean-host end-to-end journey from the artifact, no published devcontainer Feature. | Clean-host onboarding proof |
...
2. Prove the clean-host journey from the release artifact end to end, and add a single command that runs the full E2E suite with real credentials locally on demand.
