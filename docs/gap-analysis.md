...
- An isolated per-run agent home on by default: managed runs see only allowlisted agent login state through `HOME`, planted personal credentials are unreachable on the supported path, and first-login state written during a run persists durably.
