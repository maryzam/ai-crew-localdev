...
| P1 | Quality contracts are repo-centric, not project-flow-centric. | `make verify`, CI, docs checks, ADR gates, invariant gates, and source-comment gates exist. Managed-run verification and `ai-agent check` now share one bounded execution path in `internal/quality`: verify failures retain evidence logs under the same budgets, and every failure carries a deterministic class (exit, signal, start_failed) recorded in run history. Verification is still an ad hoc `--verify-cmd` with a fixed retry count; the project manifest schema exists but no contract runner consumes it yet. | Quality, autonomy |
