...
- A manifest-declared agent allowlist refused fail-closed before session creation, with per-agent model defaults recorded in run attribution.
...
|---|---|---|
| Governed project flows | Broker sessions, policy, wrappers, project overlay, and a project manifest whose quality contracts, agent allowlist, and model defaults are enforced by `ai-agent run` fail-closed. | Extend the manifest to declared secrets, services, caches, ports, approval points, and run modes, enforced by `ai-agent up --project` and `ai-agent run`. |
