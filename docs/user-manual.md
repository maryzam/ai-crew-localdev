...
The integration suite proves reuse across container replacement for both agents. Codex uses its real `login --with-api-key` and `login status` commands. Claude Code has no non-interactive online login, so the suite proves the two offline persisted-login paths it supports: an `apiKeyHelper` in `~/.claude/settings.json`, and a persisted OAuth credentials file at `~/.claude/.credentials.json`. Both survive container replacement and are recognized by `claude auth status` and `ai-agent auth status`. A live browser OAuth sign-in remains a manual first-time step; only its persistence and reuse are covered by tests.
