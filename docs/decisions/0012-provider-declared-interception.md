...
Providers are compiled in, never loaded at runtime. Shared-object or exec'd plugins inside the governance boundary are rejected because they defeat supply-chain audit. The profile registry is `internal/providers/profiles`, a contracts-only aggregation consumed by both the launcher (scrub composition and command interposition) and `internal/providers/catalog`, which constructs the provider set for the broker; a contract test fails if a provider registers in the catalog without a profile in the registry, and because the launcher consumes the same registry, a registered profile is applied at runtime by construction.
