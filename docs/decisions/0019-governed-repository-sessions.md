...
The generic managed container keeps `/workspace` as its fixed `workspaceFolder`. Its launch plan contains the private checkout as the only source-code bind mount. The broker socket remains the only host credential boundary, the root filesystem remains read-only, capabilities remain dropped, `no-new-privileges` remains enabled, and neither a container runtime socket nor the operator's Git, SSH, GitHub CLI, home, or parent workspace is mounted. Agent login state is projected according to the isolated-home decision and is scoped by configured agent identity rather than used as a general host home. The authoritative container ID returned by `devcontainer up` is captured and force-removed after the agent process exits; the devcontainer local-folder label is only a compatibility fallback when older CLI output omits the ID. Checkpointing proceeds only after this quiescence proof, and its per-workspace build context is then reclaimed.

Managed `start` and compatibility `up` enable the local Langfuse stack and sanitized trace publication by default. This is a deliberate operator-experience and data-posture choice: first use may start Postgres, ClickHouse, Redis, MinIO, and two Langfuse services, and the allowlisted trace projection includes governed run, task, repository, agent, model, outcome, usage, and cost metadata while excluding prompts, tool content, raw request bodies, credentials, and unknown fields. `--observability=false` is the single explicit opt-out and retains owner-only local run history without starting or authorizing Langfuse. The default and opt-out use the same broker boundary; backend credentials are never projected into the container.
...
Catalog identity comes from validated managed directory names, not from the metadata payload. Listing isolates corrupt or older records as `unreadable` entries so one bad record cannot hide healthy workspaces, and exact-ID removal locates only the requested directory. The exclusive `run.lock` proves live use regardless of a stale persisted state string. Non-forced removal inspects the private Git checkout directly and refuses either uncommitted changes or a `HEAD` different from the recorded base; `--force` is the sole authority to discard those recoverable facts or unreadable metadata.
