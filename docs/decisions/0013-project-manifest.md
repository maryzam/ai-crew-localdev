...
Introduce `internal/configmodel/manifest`, an in-repo project declaration discovered at `.ai-agent/manifest.json` under the repository root. Schema v1 (`ai-agent-manifest/v1`) declares quality contracts (named checks with a command and a `retry` policy of `agent` or `never`) and agents (an allowlist plus per-agent model defaults). Schema v2 (`ai-agent-manifest/v2`) keeps that surface and adds the supported project operating model: brokered resources, caches, services, ports, approvals, run modes, and token resource budgets. Parsing is strict: undeclared fields are rejected, not ignored, and v1 manifests cannot use v2 fields. Validation follows the policy pattern — `ValidateResult` with errors and warnings — and the loader enforces a one-megabyte size cap.
