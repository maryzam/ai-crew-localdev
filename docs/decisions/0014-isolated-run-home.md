...
Managed runs execute the agent with an ephemeral `HOME` by default. The launcher creates a per-run temporary directory, links in an explicit allowlist of agent login state, rewrites `HOME`, and drops the `XDG_CONFIG_HOME`, `XDG_DATA_HOME`, `XDG_STATE_HOME`, and `XDG_CACHE_HOME` overrides so they re-derive from the ephemeral home. Allowlisted directories (`.claude`, `.codex`, `.agents`) are created in the real home before linking, so directory-based first logins (`mkdir` plus writes) land durably through the link. The top-level state file (`.claude.json`) is linked even when absent so creation writes through; when a CLI saves it atomically by renaming over the link inside the ephemeral home, cleanup detects the replacement and copies the file back to the real home atomically. A directory entry replaced wholesale during a run is reported as not persisted rather than silently lost. Everything an agent writes under `HOME` outside the allowlist disappears with the run. The verify contracts inherit the same environment. Isolation is on by default on the supported path and opt-out per run.
