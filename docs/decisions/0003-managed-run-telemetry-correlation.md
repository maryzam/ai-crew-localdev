...
The persisted local schema carries an explicit `schema_version` (`internal/telemetry/schema.go`). History readers accept only events whose version matches the current `SchemaVersion` and skip anything else, including crash-truncated lines.
