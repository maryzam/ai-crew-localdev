...
Add `internal/app/adaptive` as a read-only application service above the telemetry store. `ai-agent runs analyze` supplies retained cross-project summaries and renders either human-readable or versioned JSON output. The service emits normalized provider coverage and provider-reported cost totals, then recommends action for recurring failures, retry waste, project-level high-token patterns, successful runs with missing usage, successful runs with lower-quality usage, and projects mostly run without verification.

The default policy analyzes 30 days, marks a run at 100,000 tokens, requires two matching failures, and flags weak verification when at least two runs and 80 percent of project runs lack verification. High-token runs are aggregated per project and ranked after weak verification so repeated token findings cannot hide a quality gap. The report retains at most five run IDs per finding, emits at most 20 findings, includes these budgets and its truncation count, and fails invalid policy before analysis. Analysis never writes project files, configuration, manifests, or governance policy.
