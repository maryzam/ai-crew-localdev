...
| 8 | Done in PR #102 | Implement live token budget enforcement through the native usage relay. | CLI-sourced token budget planning, explicit native measurement sources, native usage relay subscription, durable threshold events, launcher-owned process stop behavior, and fail-closed hard-budget relay handling are enforced with local tests and CI. |
| 9 | Done in PR #103 | Slim CLI imports and presentation paths after planning and execution boundaries are enforceable. | Managed run orchestration now goes through `internal/app/managedrun`, session file access goes through `internal/runtime/sessioninfo`, and dependency checks prevent `internal/cli` from importing the launcher executor. |
| 10 | In progress on `refactor/governance-command-consistency` | Reconcile `ai-agent up`, readiness, setup, doctor, and policy commands onto the shared governance resolver and capability registries. | PR #104 introduced a shared governance path/store resolver and wired setup, doctor/readiness, first-use `up` checks, Langfuse policy updates, and broker startup through it. PR #105 moved `policy init` and `policy validate` onto the same resolver and atomic store path. PR #106 centralized provider-aware policy validation in a neutral governance package so command checks, readiness/setup callers, and broker startup share the same provider registry path. This slice keeps planning boundary-aware: planning validates project manifest and compiled agent intent without rereading governance file copies, while broker startup and session creation remain the authoritative host-governance validation and enforcement seam. |
...
Move `up`, readiness, setup, doctor, and policy validation onto the same governance resolver used by broker startup and the same capability registries used by managed-run planning. This closes the current risk that setup/readiness accept a configuration shape that the broker later rejects differently, while planning stays limited to project manifest and compiled agent intent.

Checks: focused tests showing invalid identity, policy, provider, and environment contract cases fail consistently across broker startup, readiness, and setup paths, with planner tests covering manifest, compiled tool, and devcontainer-boundary cases.
