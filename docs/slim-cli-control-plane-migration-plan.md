...
| 10 | In progress on `refactor/governance-validation-service` | Reconcile `ai-agent up`, readiness, setup, doctor, and policy commands onto the shared governance resolver and capability registries. | PR #104 introduced a shared governance path/store resolver and wired setup, doctor/readiness, first-use `up` checks, Langfuse policy updates, planner identity lookup, and broker startup through it. PR #105 moved `policy init` and `policy validate` onto the same resolver and atomic store path. This slice centralizes provider-aware policy validation in a neutral governance package so command checks, readiness/setup callers, and broker startup share the same provider registry path. |
