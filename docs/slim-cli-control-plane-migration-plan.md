...
| 5 | In progress on `refactor/agent-registry` | Add first-class agent adapters for Claude and Codex. | First slice introduces the compiled agent capability registry and wires supported consumers for executable matching, native telemetry support, login-state projection, auth probes, and default guidance. Model attribution remains pending because `internal/platform/telemetry` cannot depend on agent packages; move it through the plan/event boundary instead of weakening the platform dependency rule. |
