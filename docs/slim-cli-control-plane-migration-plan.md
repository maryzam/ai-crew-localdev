...
| 8 | In progress on `refactor/live-budget-enforcement` | Implement live token budget enforcement through the native usage relay. | This slice adds explicit budget measurement sources, native usage relay subscription, durable threshold events, and launcher-owned process stop behavior for planned `native_otel` budgets. |
