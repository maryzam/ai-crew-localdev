...
| 5 | In progress on `refactor/agent-model-attribution` after PR #97 | Add first-class agent adapters for Claude and Codex. | PR #97 introduced the compiled agent capability registry and wired executable matching, native telemetry support, login-state projection, auth probes, and default guidance. This slice moves managed-run model attribution through the control-plane plan boundary so production telemetry consumes planned agent metadata instead of rediscovering it in the telemetry package. |
