...
| 3 | In progress on `refactor/plan-executor` | Convert `internal/runtime/launcher` into a devcontainer plan executor and remove duplicated resolution from it. | Executor tests prove `launcher.Launch` consumes `plan.RunPlan`, uses planned broker resources, run ID, interception, scrub policy, quality contracts, retry, cleanup, and home policy, rejects unsupported non-devcontainer entry before broker access, and no longer resolves repositories, resources, provider profiles, or verification contracts at execution time. |
