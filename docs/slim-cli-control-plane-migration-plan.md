...
| 7 | In progress on `refactor/run-event-spine` | Move run events toward an event spine and split telemetry concerns along model, local store, native relay, export projection, budget subscriber, and adaptive ledger boundaries. | First slice introduces `internal/platform/runevents` as the event DTO, JSONL store, and history projection owner while `internal/platform/telemetry` keeps compatibility wrappers. |
