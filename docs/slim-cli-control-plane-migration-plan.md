...
| 7 | In progress on `refactor/native-usage-events` | Move run events toward an event spine and split telemetry concerns along model, local store, native relay, export projection, budget subscriber, and adaptive ledger boundaries. | PR #100 introduced `internal/platform/runevents` as the event DTO, JSONL store, and history projection owner. This slice moves native usage aggregation into `runevents` so live budget enforcement can subscribe to provider-reported usage without depending on telemetry relay internals. |
