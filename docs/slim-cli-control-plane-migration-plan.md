...
| 6 | In progress on `refactor/quality-retry-plan` | Move quality contract resolution and retry policy fully into the plan. | This slice makes contract failure policy, evidence retention budget, tail budget, and retry budget explicit plan fields consumed by the executor without using quality defaults or storing derived retry attempts. |
