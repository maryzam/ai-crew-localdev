...
| 10 | In progress on `refactor/governance-policy-commands` | Reconcile `ai-agent up`, readiness, setup, doctor, and policy commands onto the shared governance resolver and capability registries. | PR #104 introduced a shared governance path/store resolver and wired setup, doctor/readiness, first-use `up` checks, Langfuse policy updates, planner identity lookup, and broker startup through it. This slice moves `policy init` and `policy validate` onto the same resolver and atomic store path. |
