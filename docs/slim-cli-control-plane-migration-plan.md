...
| 9 | In progress on `refactor/slim-cli-run` | Slim CLI imports and presentation paths after planning and execution boundaries are enforceable. | This slice removes direct `internal/cli` imports of the launcher executor for managed runs and session-file access, then adds a dependency check preventing the launcher executor from leaking back into CLI. |
