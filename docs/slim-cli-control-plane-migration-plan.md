...
| 10 | In progress on `refactor/governance-command-consistency` | Reconcile `ai-agent up`, readiness, setup, doctor, and policy commands onto the shared governance resolver and capability registries. | PR #104 introduced a shared governance path/store resolver and wired setup, doctor/readiness, first-use `up` checks, Langfuse policy updates, planner identity lookup, and broker startup through it. PR #105 moved `policy init` and `policy validate` onto the same resolver and atomic store path. PR #106 centralized provider-aware policy validation in a neutral governance package so command checks, readiness/setup callers, and broker startup share the same provider registry path. This slice makes managed-run planning fail closed on the same governance validation before runtime setup. |
