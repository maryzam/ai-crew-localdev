...
| 10 | In progress on `refactor/governance-resolver` | Reconcile `ai-agent up`, readiness, setup, and doctor onto the shared governance resolver and capability registries. | This slice introduces a shared governance path/store resolver and wires setup, doctor/readiness, first-use `up` checks, and Langfuse policy updates through it before deeper manifest and capability alignment. |
