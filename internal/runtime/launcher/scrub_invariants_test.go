...
func TestEveryProfileScrubsItsAmbientCredentials(t *testing.T) {
	for _, profile := range plannedProfiles(profiles.All()) {
		t.Run(profile.Provider, func(t *testing.T) {
			...
			result := ScrubEnv(env, plannedProfiles(profiles.All()), "/helper", "/sock", "sess-1", 3, "owner/repo", "", "")
...
func TestScrubEnvDisablesInteractiveGitCredentials(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := ScrubEnv(tt.env, plannedProfiles(profiles.All()), "/helper", "/sock", "sess", 3, "o/r", "", "")
			env := envMap(result)
...
func TestScrubEnvUsesOnlyBrokerCredentialHelper(t *testing.T) {
	env := []string{"HOME=/home/test", "PATH=/usr/bin"}
	result := ScrubEnv(env, plannedProfilesFor("o/r", "/usr/local/bin/ai-agent-credential-helper", profiles.All()), "/usr/local/bin/ai-agent-credential-helper", "/sock", "sess", 3, "o/r", "", "")
...
func TestScrubEnvReplacesParentGitConfiguration(t *testing.T) {
	...
	result := ScrubEnv(env, plannedProfiles(profiles.All()), "/helper", "/sock", "sess", 3, "o/r", "", "")
	m := envMap(result)
