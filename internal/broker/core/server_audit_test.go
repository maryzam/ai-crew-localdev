...
func TestBrokerFailsClosedWhenAuditUnavailable(t *testing.T) {
	...
	body, err := json.Marshal(api.CreateSessionRequest{AgentName: "claude", HostRepoPath: "/workspace/repo", Resources: []string{"github:repo:owner/repo"}})
	if err != nil {
		t.Fatal(err)
	}
	response := sendRequest(t, socketPath, api.Request{Method: api.MethodCreateSession, Body: body})
	if response.OK || response.Error == nil || response.Error.Code != api.ErrCodeBrokerUnavailable {
		t.Fatalf("create response = %#v", response)
	}
	health := sendRequest(t, socketPath, api.Request{Method: api.MethodHealthCheck})
	if health.OK || health.Error == nil || health.Error.Code != api.ErrCodeBrokerUnavailable {
...
func TestBrokerDenialUsesImmediateAuditFailure(t *testing.T) {
	...
	body, err := json.Marshal(api.CreateSessionRequest{AgentName: "claude"})
	if err != nil {
		t.Fatal(err)
	}
	response := sendRequest(t, socketPath, api.Request{Method: api.MethodCreateSession, Body: body})
...
func TestBrokerDoesNotMintWithoutDurableAuditIntent(t *testing.T) {
	b, socketPath, cleanup := testBroker(t)
	defer cleanup()
	createBody, err := json.Marshal(api.CreateSessionRequest{AgentName: "claude", HostRepoPath: "/workspace/repo", Resources: []string{"github:repo:owner/repo"}})
	if err != nil {
		t.Fatal(err)
	}
	created := sendRequest(t, socketPath, api.Request{Method: api.MethodCreateSession, Body: createBody})
	if !created.OK {
	...
	b.audit = &testAuditSink{err: errors.New("storage failed")}
	mintBody, err := json.Marshal(api.CredentialRequest{SessionID: session.SessionID, BindSecret: session.BindSecret, CredentialType: githubcontract.CredentialType, Resource: "github:repo:owner/repo"})
	if err != nil {
		t.Fatal(err)
	}
	response := sendRequest(t, socketPath, api.Request{Method: api.MethodMintCredential, Body: mintBody})
	if response.OK || response.Error == nil || response.Error.Code != api.ErrCodeBrokerUnavailable {
...
func TestBrokerRevocationRemainsFailClosedWhenAuditFails(t *testing.T) {
	...
	b.audit = &testAuditSink{err: errors.New("storage failed")}
	revokeBody, err := json.Marshal(api.RevokeSessionRequest{SessionID: session.SessionID})
	if err != nil {
		t.Fatal(err)
	}
	response := sendRequest(t, socketPath, api.Request{Method: api.MethodRevokeSession, Body: revokeBody})
	if response.OK || response.Error == nil || response.Error.Code != api.ErrCodeBrokerUnavailable {
...
func TestBrokerExpirationRequiresDurableIntent(t *testing.T) {
	newExpiredBroker := func(t *testing.T, audit AuditSink) (*Broker, string) {
		...
		session, _, err := store.Create(api.CreateSessionRequest{AgentName: "agent", Resources: []string{"github:repo:owner/repo"}}, uint32(1000))
		if err != nil {
			t.Fatal(err)
		}
