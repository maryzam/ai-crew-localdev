...
func (h *safetyHarness) mintFor(agent, resource string) api.Response {
	h.t.Helper()
	...
	resp := sendRequest(h.t, h.sockPath, api.Request{Method: api.MethodCreateSession, Body: body})
	if !resp.OK {
	...
	return sendRequest(h.t, h.sockPath, api.Request{Method: api.MethodMintCredential, Body: mintBody})
}

func extractMintedToken(t *testing.T, resp api.Response) string {
	t.Helper()
	if !resp.OK {
...
func TestBrokerRejectsCredentialTypeResourceMismatch(t *testing.T) {
	...
	resp := sendRequest(t, h.sockPath, api.Request{Method: api.MethodCreateSession, Body: body})
	if !resp.OK {
	...
	mintResp := sendRequest(t, h.sockPath, api.Request{Method: api.MethodMintCredential, Body: mintBody})
	if mintResp.OK {
		t.Fatal("expected mismatch to be rejected")
	}
	if mintResp.Error.Code != api.ErrCodeUnknownCredType {
		t.Errorf("error code = %q, want %q", mintResp.Error.Code, api.ErrCodeUnknownCredType)
	}
	if h.gh.calls(42) != 0 {
...
func TestBrokerMintAfterReloadRemovingResourceIsRejected(t *testing.T) {
	...
	if mintResp.Error.Code != api.ErrCodeResourceNotAllowed {
		t.Errorf("error code = %q, want %q", mintResp.Error.Code, api.ErrCodeResourceNotAllowed)
	}
	if h.gh.calls(42) != 0 {
	...
	for i := 0; i < 4; i++ {
		...
		go func() {
			...
			for {
				...
				r := sendRequest(t, h.sockPath, api.Request{Method: api.MethodMintCredential, Body: mintBody})
				switch {
				case r.OK:
				case r.Error.Code == api.ErrCodeBrokerUnavailable:
				case r.Error.Code == api.ErrCodeRateLimited:
				default:
					mintErrors.Add(1)
				}
			}
